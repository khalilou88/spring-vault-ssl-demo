...
public class ServerAClientService {
    ...
    public ServerAClientService(RestTemplateBuilder restTemplateBuilder, SslBundles sslBundles) {
        ...
        this.restTemplate = restTemplateBuilder
                .setSslBundle(sslBundles.getBundle("server-a-ssl"))
                .build();
    }
    ...
    public MessageResponse sendMessageToServerB(String message) {
        ...
        MessageRequest request =
                new MessageRequest(message, LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));

        try {

            MessageResponse response =
                    restTemplate.postForObject(serverBUrl + "/api/v1/message", request, MessageResponse.class);
